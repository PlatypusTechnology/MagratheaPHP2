...
This repo has an AI-built documentation site at `docs/` (see
`docs/CLAUDE.md` for its architecture). **Any change to `src/` should be
reflected there in the same session**, not left for later:

- **Method/class signatures never need manual updates** — `docs/lib/Reflector.php`
...
- **Narrative prose can go stale and needs a human/AI pass.** If you add, rename, move, or
  ...
  `docs/mds/*.md` (rendered on the corresponding `documentation` page)
  still describes reality — and update it if not. `docs/lib/DocMap::Freshness()` will
  ...
  "here's what's wrong" (see the `docs/mds/core/config.md` rewrite from
  2026-07-20, where
...
- **New top-level classes/topics need a `lib/DocMap.php` entry** (`docs/lib/DocMap.php`)
...
- **`skills.MD`** (the AI-facing coding cookbook, downloadable from the site as `skill.md`) can
  ...
  against the actual current source too, not just against `docs/mds/`.
...
`docs/CLAUDE.md`: always ask before implementing, and be critical of any
new idea or change.
